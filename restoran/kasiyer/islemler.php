...
$(document).ready(function(){
	...
	$('#hesapalbtn2').click(function(){
		$.ajax({
				type :"POST",
				url  :'islemler.php?islem=hesap2',
				data :$('#hesapal').serialize(),
				success : function(donen_veri){
					$('#hesapal').trigger("reset");
						window.location.reload();
...
function sorgum2($vt,$sorgu,$tercih) {
	...
	switch($islem):
		...
		case "hesap" :
			if(!$_POST):
				...
				$gelen = sorgum2($db, "select ad from masalar where id=$masaid", 1)->fetch_assoc();
                $masaad = $gelen["ad"];
				
				$gelen3 = sorgum2($db, "select musteriid from musterimasa where masaid=$masaid", 1)->fetch_assoc();
                $musteriid = $gelen3["musteriid"];
				
				$gelen2 = sorgum2($db, "select * from musteri where id=$musteriid", 1)->fetch_assoc();
                $adsoyad = $gelen2["adsoyad"];
				...
				while($don=$verilericek->fetch_assoc()) :
					...
					$g=$don["adet"];		
					$bugun = date ("Y-m-d");
				$raporekle="insert into rapor (masaid,masaad,garsonid,urunid,musteriid,musveri,urunad,urunfiyat,adet,odeme,tarih)
				VALUES($a,'$masaad',$b,$c,$d,'$adsoyad','$e',$f,$g,'NAKİT','$bugun')";
				$raporekles=$db->prepare($raporekle);
				$raporekles->execute();
				
				$urunebak = sorgum2($db, "select stok from urunler where id=$b", 1);
				...
				$silme->execute();
				$sorgu2="delete  from musterimasa where masaid=$masaid";
	...
	case "hesap2" :
			if(!$_POST):
				...
				while($don=$verilericek->fetch_assoc()) :
					...
					$bugun = date ("Y-m-d");
				$raporekle="insert into rapor (masaid,masaad,garsonid,urunid,musteriid,musveri,urunad,urunfiyat,adet,odeme,tarih)
				VALUES($a,'$masaad',$b,$c,$d,'$adsoyad','$e',$f,$g,'KREDİ','$bugun')";
				$raporekles=$db->prepare($raporekle);
				$raporekles->execute();
	 ...
	 $id=htmlspecialchars($_GET["id"]);
		...
		uyari("Henüz Müşteri Yok","primary");
			...
			<tr class="bg-dark text-white text-center">
				...
				<tr class="bg-dark text-white text-center">
				<td class="col-md-6 font-weight-bold text-success">'.$musteri.'</td>
			</tr>
			</tbody></table>
			...
			<input type="button" id="hesapalbtn1" name="nakit" value="NAKİT" class="btn btn-success btn-block mt-4"/>
			</form>
			</div>
			...
			<form id="hesapal">
			<input type="hidden" name="masaid" value="'.$masaid.'"/>
			<input type="button" id="hesapalbtn2" name="kredi" value="KREDİ KARTI" class="btn btn-danger btn-block mt-4"/>
			</form>
			</div>
		...
		if($var->num_rows!=0) :
			...
			$ekle="insert into siparis (masaid,garsonid,urunid,urunad,urunfiyat,adet,musteri) VALUES
			($masaid,$garsonidyaz,$urunid,'$urunad',$urunfiyat,$adet,'$musteri')";
	...
	$var=sorgum2($db,"select * from kasiyer where ad='$ad' and sifre='$sifre'",1);
	
	if ($var->num_rows==0) :
	echo'<div class="alert alert-danger text-center">Bilgiler uyuşmuyor</div>';
	else:
	$kasiyer=$var->fetch_assoc();
	$kasiyerid=$kasiyer["id"];
	sorgum2($db,"update kasiyer set durum=1 where id=$kasiyerid",1);
	...
	case "cikis":
	sorgum2($db,"update kasiyer set durum=0",1);
